...
// configuration
clearscreen.
parameter include_mono is 0.

runoncepath("lib_physics.ks").
runoncepath("lib_ship_steering.ks").

gear on.
sas off.
declare global fuelMass to 0.
getFuelMass(include_mono).
// physics
...
// LOCK STEERING TO HEADING(compute_heading(srfretrograde:forevector),2).
lock steering to srfretrograde.
lock time1 to stoptime.
lock time2 to impacttime.
//add a little slop with +0.05
lock idealthrottle to (time1+0.025)/time2.                  // hoverslam throttling setting
...
declare function getFuelMass {
    ...
    for rsc in fuels {
        ...
        else if (rsc:name = "OXIDIZER"){
            set fuelMass to fuelMass + rsc:density.
        }
        else if (include_mono and rsc:name = "MONOPROPELLANT") {
            set fuelMass to fuelMass + rsc:density.
        }
