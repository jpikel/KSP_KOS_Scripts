...
function circ_ap {
    ...
    // Fail-safe: If eta:apoapsis is huge (e.g. we just passed it and it wrapped around to the next orbit),
    // we need to burn immediately before we fall back into the atmosphere!
    local node_time to time:seconds + eta:apoapsis.
    if ship:orbit:period > 0 and eta:apoapsis > (ship:orbit:period / 2) {
        set node_time to time:seconds + 15.
    }

    add node(node_time, 0, 0, circdv).
}
